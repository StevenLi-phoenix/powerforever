...
class calculate:
    def __init__(self, avg_data=True):
        ...
        self.threadLimiter = threading.BoundedSemaphore(50)

        self.base = "/Users/lishuyu/PycharmProjects/solarPowerEstimate/"
    ...
    def evaluate(self, batteries=None):
        ...
        for hour in range(320 * 24):
            ...
            if charge > discharge:
                ...
                if pull > 0:
                    self.log.write(
                        f"{charge}, {discharge}, {sum([i.capacity for i in batteries])}, note: battery is later for one")
                    ...
                    if status:
                        ...
                        status = False
                        self.log.write(
                            f"No avaliable energy on day:{hour // 24}, {sum([i.capacity for i in batteries])}, count_increase, set status to false")
        ...
        # return year_round_status, current_cap, max_nopower_count
